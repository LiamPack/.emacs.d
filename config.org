...
({~mct~, ~embark~, ~consult~}, ~yasnippet~, ...). Simple packages allow composability:
...
** personal theme
Some modifications to the built-in theme I'm playing with. Playing around with some [[https://protesilaos.com/codelog/2020-08-28-notes-emacs-theme-devs/][advice
from Prot]] in building my own kind of theme. Right now its a direct rip of the [[https://github.com/rocher/ubuntu-theme][ubuntu-theme]]
package (last updated 2015 as of writing [2022-03-06 Sun 15:16]).
#+begin_src emacs-lisp
...
#+end_src
** mct        -- minimalist minibuffer completion visualization
...
(when (not (directory-name-p "~/.emacs.d/consult-bibtex"))
...
** USLS
#+begin_src emacs-lisp
(when (not (directory-name-p "~/.emacs.d/usls"))
  (shell-command "cd ~/.emacs.d/elpa && git clone https://gitlab.com/protesilaos/usls.git"))

(add-to-list 'load-path "~/.emacs.d/elpa/usls/" t)
(lp-emacs-builtin-package 'usls
...
(lp-emacs-elpa-package 'paredit
  (defun lp--activate-paredit ()
    "Activates `paredit-mode'"
    (interactive)
    (paredit-mode +1))

  (add-hook 'emacs-lisp-mode-hook #'lp--activate-paredit)
  (add-hook 'lisp-mode-hook #'lp--activate-paredit)
  (add-hook 'scheme-mode-hook #'lp--activate-paredit)
  (add-hook 'sly-mode-hook #'lp--activate-paredit))
...
(lp-emacs-elpa-package 'geiser
  (add-hook 'geiser-mode-hook #'lp--activate-paredit))
