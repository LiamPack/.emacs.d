...
=ibuffer= remains a time-tested classic. It does its job well: logical grouping of buffers
with some enhanced information over =list-buffers=, packaged with advanced marking and
acting over the listed buffers (much like =dired{,-x}=).
...
#+end_src
** completion frameworks and utilities (abbrev, dabbrev, corfu)
#+begin_src emacs-lisp
(use-package abbrev
  :straight (:type built-in)
  :config
